...
| P1 negative-control declaration gate | synthetic-verified | Exact seven-control family covers blocked score permutation, matched random modules, endpoint specificity, attendance/censoring, and site/batch/QC. One valid fixture passes and nine altered/post-hoc/rescuing routes fail closed. |
| External-review brief | artifact and claim-boundary checked | Skeptic-facing account leads with no progression state/target, enumerates falsifiers and conditional design bounds, and defines the P1->P2->P3 path without therapeutic inflation. |
...
| Consolidated progression regression suite | 37/37 commands and 205/205 invariants pass | Fast gates, numerical references, negative claim boundaries, candidate-identity/release/result-interpretation/external-review checks, artifact navigation/compute accounting, acquisition/request synthesis, negative-control, confirmation-error/provenance, precision, and power boundaries, provenance/structure, and repository guards execute from one command. No biological claim. |
...
| Role-based progression artifact index | complete and machine-verified | Forty-eight unique committed artifacts route medical, acquisition, analysis-operator, and method-review readers with path checks and explicit claim authority; indexing never upgrades evidence. |
...
## Progression Negative-Control Declaration Gate

Status: **synthetic-verified, blind, and non-rescuing**.

The fixed seven-control family is documented in
`docs/validation/PROGRESSION_NEGATIVE_CONTROL_GATE_V54.md` and enforced by
`scripts/v54_progression_negative_control_gate.py`. It includes subject-blocked
score permutation, a 100-member matched random-module max-T bank, transient/
relapse-associated/pre-index endpoint diagnostics, attendance/censoring process
testing, and residual site/batch/quality testing.

Ten synthetic declarations behave as required: one exact family passes and
nine missing, under-calibrated, post-access, rescuing, selectively reportable,
or action-altered variants fail closed. Every failure has a fixed consequence:
invalid primary inference, event-time failure, technical/transport failure, or
progression-specificity downgrade. Clean controls retain the primary's own
grade only; they never validate, rescue, or upgrade it. These fixtures test
process behavior and contain no patient data or empirical control result.
...
A separately frozen upper-range extension then generated 72,000 additional
cohorts at `n=1,800`, `2,100`, and `3,000`. Under the strong homogeneous HR-1.5
and 30%-event assumptions, balanced allocation first reaches precision
readiness at `n=1,800` (strict probability `0.832`, minimum seed `0.830`,
median weakest-site events `102`, widest CI half-width `0.195`). The 60/30/10
design does not pass until `n=3,000`. No 15%-event route and no HR-1.3 route
passes through `n=3,000`. The full conditional lookup is in
...
## Enrollment Inflation And Analyzable Events

Status: **conditional planning lookup complete; no universal sample size**.

Executable audit:

- frozen plan: `docs/plans/PROGRESSION_ENROLLMENT_INFLATION_V54.md`
- script: `scripts/v54_progression_enrollment_inflation.py`
- results: `analysis/v54_progression_enrollment_inflation/`

The seeded audit evaluated 122,805,000 synthetic cohort replicates. Equal site
quotas were tested over 54 combinations of follow-up loss, score missingness,
event-confirmation loss, and 0.15/0.30 event probabilities. A candidate must
place the 95% Wilson lower assurance bound at or above 0.90 in all three seeds
for 450 analyzable participants, 135 confirmed events, and per-site floors of
150 analyzable participants and 10 events.

Under the reference planning assumptions of 10% loss in each channel and event
probability 0.30, the first equal-quota candidate is gross `690` (`230/site`,
`1.53x` the analyzable target). With 20% loss in all channels it is `990`
(`330/site`, `2.20x`). Holding losses at 10% but lowering event probability to
0.15 requires `1,380` (`460/site`, `3.07x`) because confirmed events, not
participant count, become limiting.

Passive recruitment illustrates why inflation is not transport repair. Under
the 10%/0.30 scenario, balanced-in-expectation recruitment clears arithmetic
floors near 690, `45/35/20` near 1,035, and `60/30/10` near 2,055. The unequal
routes remain `OUTSIDE_TESTED_BALANCED_REFERENCE` after clearing those floors.
Only explicit equal quotas align with the tested design, and realized data must
still pass all site transport diagnostics. These values condition on independent
synthetic losses; they are not observed MS rates, guarantees, or universal
sample-size recommendations.

## Weaker-Effect Progression Power

Status: **clean-reference sensitivity complete; n=450 is not universal**.

The frozen extension (`docs/plans/PROGRESSION_WEAKER_EFFECT_POWER_V54.md`;
`scripts/v54_progression_weaker_effect_power.py`) generated 90,000 synthetic
balanced-site cohorts through analyzable `n=1,500`. The within-site
standardized, site-stratified null family calibrates: maximum `0.0572`, Wilson
lower bound `0.0474`, and ten-cell family tail `0.611`.

At 30% events, HR 1.3 per latent SD first passes at `n=900` (conditional gross
`1,380` under the established 10%-per-channel losses); HR 1.5 and 1.7 pass at
`n=450` (gross `690`). At 15% events, HR 1.3 requires `n=1,500` (gross `2,301`),
HR 1.5 requires `n=600` (gross `921`), and HR 1.7 passes at `n=450`. HR 1.2
does not reach the rule by `n=1,500`: positive-call probability is `0.529` at
15% events and `0.789` at 30%.

These are latent-effect assumptions under clean complete ascertainment. The
true effect is unknown, and informative attendance or other departures can
invalidate the route rather than merely reduce power. The result narrows the
reference-design communication: 450 is defensible only near the stronger
assumptions, not as a universal target.
...
## Nonlinear Diagnostic Receipt Gate

Status: **synthetic-verified; fixed and non-rescuing**.

The additive contract
(`docs/validation/PROGRESSION_NONLINEAR_DIAGNOSTIC_GATE_V54.md`;
`scripts/v54_progression_nonlinear_diagnostic_gate.py`) tests ten synthetic
declarations. Corrected asymptotic and disjoint empirical-null routes pass.
Missing quadratic coverage, a changed threshold, uncorrected alpha, prior score
access, post-result rescue/replacement, overlapping calibration, and a missing
calibration source fail closed.

The primary remains one standardized linear coefficient and must be reported
regardless. The threshold at `z > 0.674`, `tanh(1.5z)`, and two-degree
linear-plus-quadratic omnibus are frozen blind. A diagnostic can only queue a
separately pre-registered future model; passing does not imply a nonlinear MS
risk shape or progression association.

## Linear-Effect Misspecification

Status: **one fixed synthetic design warning; diagnostics remain non-rescuing**.

The audit (`docs/plans/PROGRESSION_LINEAR_MISSPECIFICATION_V54.md`;
`scripts/v54_progression_linear_misspecification.py`) uses 18,000 disjoint
null-calibration cohorts and 108,000 evaluation cohorts. It compares the one-
coefficient primary with predeclared high-threshold, saturated-score, and
linear-quadratic diagnostics under corrected multiplicity. All five evaluation
null families calibrate after independent empirical p-thresholding.

At `n=320`, the U-shaped crossing generator meets the frozen material-miss
rule: primary linear detection is `0.105`, corrected linear-quadratic detection
is `0.891`, aggregate gain is `0.786`, and every-seed gain is at least `0.779`.
At `n=180`, the same diagnostic reaches only `0.637` and does not pass. The
linear route is more sensitive than the fixed diagnostic for high-threshold and
monotone-saturation patterns; the inverted-U diagnostic remains underpowered.

The initial asymptotic run failed four null families. A disjoint calibration
bank was added before interpreting shapes, and evaluation replication was
expanded from 800 to 3,000/seed after its first calibrated null remained noisy.
Those failed controls and repairs are documented. The surviving result says
only that a single linear coefficient can miss a sufficiently strong symmetric
synthetic risk shape; it does not show such a shape in MS and cannot rescue a
failed primary analysis post hoc.
...
## Endpoint-Confirmation Error Sensitivity

Status: **score-linked confirmation error is a fail-closed inference
boundary**.

The frozen audit
(`docs/plans/PROGRESSION_CONFIRMATION_ERROR_V54.md`;
`scripts/v54_progression_confirmation_error.py`) generated 230,400 synthetic
cohorts across eight endpoint-confirmation mechanisms. Four null families
calibrate: complete confirmation, independent missed confirmation,
risk-only missed confirmation, and independent false confirmation. Under those
bounded assumptions, HR 1.5 reaches the design rule at `n=900` with 15% events
or `n=450` with 30% events.

Score-linked adjudication is method-invalid, not merely underpowered.
Score-dependent missed confirmation yields a maximum null call rate of `0.329`
in the protective direction, joint score/risk missed confirmation reaches
`0.407` protective, and score-dependent false confirmation reaches `0.335`
harmful. The mixed family is also invalid (`0.104`, predominantly protective).
Consequently, confirmation provenance and assessor/process blindness are
mandatory before score access; an unknown dependence route fails closed. These
synthetic mechanisms estimate no real MS confirmation-error rate and create no
...
## P1 Post-Result Interpretation Composition

Status: **synthetic-verified; bounded pass is not therapeutic evidence**.

The post-result gate
(`docs/validation/PROGRESSION_P1_RESULT_INTERPRETATION_GATE_V54.md`;
`scripts/v54_progression_result_interpretation_gate.py`) implements the four
classes already frozen in the blinded P1/P2 contract. It recomputes the hash of
the actual frozen plan file, checks package/release identity and execution
order, and derives interval relationships from that plan's null and
minimum-material boundaries rather than selecting new thresholds.

All 17 synthetic packets route as expected: two bounded passes, one pass with
progression-specificity downgrade, two failures, seven inconclusive outcomes,
and five invalid outcomes. Process-control failures invalidate; endpoint-
specificity controls can only downgrade; clean controls never upgrade. An
estimate exactly on the null is correctly inconclusive when its interval spans
the null and material boundaries, rather than being mislabeled wrong-direction.
A bounded pass means predictive-association transport under the declared design
only, not mechanism, target, treatment effect, efficacy, or evidence of halting
progression.

## Role-Based Artifact Navigation

Status: **48/48 artifacts resolve; navigation changes no evidence grade**.

`docs/validation/PROGRESSION_ARTIFACT_INDEX_V54.md` separates four reader
paths: eight medical-interpretation artifacts, nine acquisition artifacts,
...
claim boundaries. All 48 committed entries pass.
...
information lock, and analysis release retain execution authority only; none
is presented as progression evidence, validation, or therapeutic support.

## Compute Accounting

Status: **complete; categories are explicitly non-additive**.

`docs/validation/PROGRESSION_COMPUTE_LEDGER_V54.md` reads 46 committed summary
artifacts and records 57 count rows. It separates 3,348,600 unique model-fit
synthetic cohorts (including explicitly disjoint calibration cohorts),
122,805,000 lightweight enrollment/loss/event planning draws, 3,875,400 method
or estimand evaluations that reuse cohorts, 2,700,000 permutation/donor-wild
draws over held data, and 187 discrete gate fixtures.

No grand total is reported: adding those units would double-count route reuse
and equate lightweight planning draws with fitted simulations. These counts
characterize computational method work only; they are neither patient evidence
nor a measure of biological support.
...
The final suite executes 37 checks and asserts 205 committed artifact/claim
