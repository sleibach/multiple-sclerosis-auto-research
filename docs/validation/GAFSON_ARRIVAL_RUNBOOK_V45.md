...
| 3 | outcome dictionary | complete `docs/validation/input_schemas/V45_outcome_label_dictionary_template.tsv` as `metadata/outcome_label_dictionary.tsv` before scoring; validate with `scripts/v45_outcome_label_dictionary_validator.py` | Outcome mapping | NEDA-4 orientation, assessment window, missingness rules, and binary mapping frozen | stop; missing/ambiguous endpoint blocker |
