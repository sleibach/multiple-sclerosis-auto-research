# V48 Source-Terms Coverage

Status: provenance/navigation only. External records remain `external-unverifiable` or `external-verifiable` as tagged; source-terms metadata is not project-grounded evidence.

- records checked: `63`
- records with source_terms: `32`
- records missing optional source_terms: `31`
- NOT project grounded marker: `NOT_PROJECT_GROUNDED`

## Redistribution Coverage

| redistribution_allowed | count |
|---|---:|
| `metadata_only` | 32 |
| `missing` | 31 |

## Records

| record | class | source | source_terms | redistribution | checked | terms | marker |
|---|---|---|---|---|---|---|---|
| `resource.arrayexpress_biostudies.functional_genomics.2026-06-13` | `external-unverifiable` | https://www.ebi.ac.uk/arrayexpress/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.biostudies.database.2026-06-13` | `external-unverifiable` | https://www.ebi.ac.uk/about/teams/biostudies/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.clinicaltrials_gov.2026-06-13` | `external-unverifiable` | https://clinicaltrials.gov/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.dailymed.drug_labels.2026-06-13` | `external-unverifiable` | https://dailymed.nlm.nih.gov/ | `present` | `metadata_only` | `2026-06-14` | https://dailymed.nlm.nih.gov/dailymed/about-dailymed.cfm | `NOT_PROJECT_GROUNDED` |
| `resource.disgenet.platform.2026-06-13` | `external-unverifiable` | https://disgenet.com/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.dryad.repository.2026-06-13` | `external-unverifiable` | https://datadryad.org/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.ega.controlled_genomics.2026-06-13` | `external-unverifiable` | https://ega-archive.org/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.ema.medicines_database.2026-06-13` | `external-unverifiable` | https://www.ema.europa.eu/en/medicines | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.ena.sequence_archive.2026-06-13` | `external-unverifiable` | https://www.ebi.ac.uk/ena/browser/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.europe_pmc.literature.2026-06-13` | `external-unverifiable` | https://europepmc.org/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.fda.fdalabel_drug_labeling.2026-06-13` | `external-unverifiable` | https://www.fda.gov/science-research/bioinformatics-tools/fdalabel-full-text-search-drug-product-labeling | `present` | `metadata_only` | `2026-06-14` | https://www.fda.gov/about-fda/about-website/website-policies | `NOT_PROJECT_GROUNDED` |
| `resource.figshare.repository.2026-06-13` | `external-unverifiable` | https://info.figshare.com/about/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.geo.functional_genomics.2026-06-13` | `external-unverifiable` | https://www.ncbi.nlm.nih.gov/geo/ | `present` | `metadata_only` | `2026-06-14` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `resource.gwas_catalog.ms.2026-06-13` | `external-unverifiable` | https://www.ebi.ac.uk/gwas/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.imsgc.publications.2026-06-13` | `external-unverifiable` | https://imsgc.net/publications/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.ms_society_uk.types_of_ms.2026-06-13` | `external-unverifiable` | https://www.mssociety.org.uk/about-ms/types-of-ms | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.msbase.registry.2026-06-13` | `external-unverifiable` | https://www.msbase.org/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.msda.catalogue.2026-06-13` | `external-unverifiable` | https://msda.emif-catalogue.eu/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.msgd.database_commons.2026-06-13` | `external-unverifiable` | https://ngdc.cncb.ac.cn/databasecommons/database/id/9285 | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.msif.atlas_of_ms.2026-06-13` | `external-unverifiable` | https://atlasofms.org/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.narcoms.registry.2026-06-13` | `external-unverifiable` | https://www.narcoms.org/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.national_ms_society.dmt_reference.2026-06-13` | `external-unverifiable` | https://www.nationalmssociety.org/for-professionals/for-healthcare-professionals/managing-and-treating-ms/disease-modification | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.national_ms_society.types_of_ms.2026-06-13` | `external-unverifiable` | https://www.nationalmssociety.org/understanding-ms/what-is-ms/types-of-ms | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.nhs_england.ms_dmt_algorithm.2026-06-13` | `external-unverifiable` | https://www.england.nhs.uk/wp-content/uploads/2024/03/treatment-algorithm-for-multiple-sclerosis-disease-modifying-therapies-july-23.pdf | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.nice.ng220_ms_management.2026-06-13` | `external-unverifiable` | https://www.nice.org.uk/guidance/ng220 | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.ninds.ms_common_data_elements.2026-06-13` | `external-unverifiable` | https://www.commondataelements.ninds.nih.gov/Multiple%20Sclerosis | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.open_targets.platform.2026-06-13` | `external-unverifiable` | https://platform-docs.opentargets.org/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.osf.repository.2026-06-13` | `external-unverifiable` | https://www.cos.io/products/osf | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `resource.pubmed.literature.2026-06-13` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/ | `present` | `metadata_only` | `2026-06-14` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `resource.sra.sequence_read_archive.2026-06-13` | `external-unverifiable` | https://www.ncbi.nlm.nih.gov/sra | `present` | `metadata_only` | `2026-06-14` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `resource.zenodo.repository.2026-06-13` | `external-unverifiable` | https://about.zenodo.org/ | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.ncbi_gene.cd74_mhc2_mif_molecular_context.2026-06-28` | `external-unverifiable` | https://www.ncbi.nlm.nih.gov/gene/972 | `present` | `metadata_only` | `2026-06-28` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `claim.gwas_catalog.chr1_rs7522462_kif21b_gpr25_ms.2026-06-28` | `external-unverifiable` | https://www.ebi.ac.uk/gwas/rest/api/associations/search/findByRsId?rsId=rs7522462&projection=associationBySnp | `present` | `metadata_only` | `2026-06-28` | https://www.ebi.ac.uk/gwas/docs/about | `NOT_PROJECT_GROUNDED` |
| `claim.jcc_2024.crohn_pants_antitnf_interferon_modules.2026-06-28` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/37776235/ | `present` | `metadata_only` | `2026-06-28` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `claim.diebold_2022.dmf_high_dimensional_immune_monitoring_context.2026-06-28` | `external-unverifiable` | https://www.pnas.org/doi/10.1073/pnas.2205042119 | `present` | `metadata_only` | `2026-06-28` | https://www.pnas.org/page/about/permissions | `NOT_PROJECT_GROUNDED` |
| `claim.carlstrom_2019.dmf_monocyte_ros_response_context.2026-06-28` | `external-unverifiable` | https://www.nature.com/articles/s41467-019-11139-3 | `present` | `metadata_only` | `2026-06-28` | https://www.nature.com/ncomms/about/open-access | `NOT_PROJECT_GROUNDED` |
| `claim.gafson_2018.dmf_pbmc_neda4_transcriptome_context.2026-06-28` | `external-unverifiable` | https://pmc.ncbi.nlm.nih.gov/articles/PMC6168332/ | `present` | `metadata_only` | `2026-06-28` | https://creativecommons.org/licenses/by/4.0/ | `NOT_PROJECT_GROUNDED` |
| `claim.sanchez_sanz_2023.dmf_pbmc_response_signature_context.2026-06-28` | `external-unverifiable` | https://www.omicsdi.org/dataset/geo/GSE235357 | `present` | `metadata_only` | `2026-06-28` | https://www.ncbi.nlm.nih.gov/geo/info/disclaimer.html | `NOT_PROJECT_GROUNDED` |
| `claim.dailymed.dimethyl_fumarate_mechanism_context.2026-06-13` | `external-unverifiable` | https://dailymed.nlm.nih.gov/dailymed/lookup.cfm?setid=6c42107a-2a42-4263-97b6-ae0d7675c92a | `present` | `metadata_only` | `2026-06-14` | https://dailymed.nlm.nih.gov/dailymed/about-dailymed.cfm | `NOT_PROJECT_GROUNDED` |
| `claim.dailymed.natalizumab_mechanism_context.2026-06-13` | `external-unverifiable` | https://dailymed.nlm.nih.gov/dailymed/drugInfo.cfm?setid=c5fdde91-1989-4dd2-9129-4f3323ea2962 | `present` | `metadata_only` | `2026-06-14` | https://dailymed.nlm.nih.gov/dailymed/about-dailymed.cfm | `NOT_PROJECT_GROUNDED` |
| `claim.dailymed.ocrelizumab_mechanism_context.2026-06-13` | `external-unverifiable` | https://dailymed.nlm.nih.gov/dailymed/drugInfo.cfm?setid=9da42362-3bb5-4b83-b4bb-b59fd4e55f0d | `present` | `metadata_only` | `2026-06-14` | https://dailymed.nlm.nih.gov/dailymed/about-dailymed.cfm | `NOT_PROJECT_GROUNDED` |
| `claim.biorxiv_2026.ebv_anti_cns_bcell_apc_ms_context.2026-06-28` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/41727017/ | `present` | `metadata_only` | `2026-06-28` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `claim.nature_2022.ebna1_glialcam_crossreactive_bcells_ms.2026-06-28` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/35073561/ | `present` | `metadata_only` | `2026-06-28` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `claim.science.ebv_ms_longitudinal_risk_context.2026-06-14` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/35025605/ | `present` | `metadata_only` | `2026-06-14` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.iuphar.gpr25_orphan_gpcr_context.2026-06-28` | `external-unverifiable` | https://www.guidetopharmacology.org/services/targets?name=GPR25 | `present` | `metadata_only` | `2026-06-28` | https://www.guidetopharmacology.org/about.jsp#terms | `NOT_PROJECT_GROUNDED` |
| `claim.nature.ms_ibd_gpr25_context.2026-06-14` | `external-unverifiable` | https://pmc.ncbi.nlm.nih.gov/articles/PMC8463615/ | `present` | `metadata_only` | `2026-06-14` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.jimmunol_2014.hla_dra1_cd74_mif_eae_context.2026-06-28` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/24683185/ | `present` | `metadata_only` | `2026-06-28` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `claim.frontiers.uc_tofacitinib_mhc_stat1_context.2026-06-14` | `external-unverifiable` | https://pmc.ncbi.nlm.nih.gov/articles/PMC9152176/ | `present` | `metadata_only` | `2026-06-14` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.nat_immunol_2024.imid_antitnf_single_cell_atlas.2026-06-28` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/39438660/ | `present` | `metadata_only` | `2026-06-28` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `claim.jmg_2010.kif21b_ms_susceptibility_replication.2026-06-28` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/20587413/ | `present` | `metadata_only` | `2026-06-28` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `claim.probast_tripod.prediction_model_validation_context.2026-06-14` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/30596875/ | `present` | `metadata_only` | `2026-06-14` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.open_targets.direction_tractability_context.2026-06-14` | `external-unverifiable` | https://www.annualreviews.org/content/journals/10.1146/annurev-biodatasci-102523-103838 | `present` | `metadata_only` | `2026-06-14` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.plos.ms_mhc_independent_effects_context.2026-06-14` | `external-unverifiable` | https://pmc.ncbi.nlm.nih.gov/articles/PMC3836799/ | `present` | `metadata_only` | `2026-06-14` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.eji_2018.mif_cd74_bcell_ms_context.2026-06-28` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/30160778/ | `present` | `metadata_only` | `2026-06-28` | https://www.ncbi.nlm.nih.gov/home/about/policies/ | `NOT_PROJECT_GROUNDED` |
| `claim.cshperspect.ms_biomarker_heterogeneity_context.2026-06-14` | `external-unverifiable` | https://pmc.ncbi.nlm.nih.gov/articles/PMC6396336/ | `present` | `metadata_only` | `2026-06-14` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.ms_society_uk.ppms_course_context.2026-06-13` | `external-unverifiable` | https://www.mssociety.org.uk/about-ms/types-of-ms | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.national_ms_society.rrms_course_context.2026-06-13` | `external-unverifiable` | https://www.nationalmssociety.org/understanding-ms/what-is-ms/types-of-ms | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.national_ms_society.spms_course_context.2026-06-13` | `external-unverifiable` | https://www.nationalmssociety.org/understanding-ms/what-is-ms/types-of-ms/secondary-progressive-ms | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.ms_ibd.treatment_transfer_caution_context.2026-06-14` | `external-unverifiable` | https://www.nature.com/articles/s41467-021-25768-0 | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.nature.ms_uc_greater_genetic_correlation_context.2026-06-14` | `external-unverifiable` | https://www.nature.com/articles/s41467-021-25768-0 | `missing_optional` | `missing` | `missing` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.ard.ra_sle_pregnancy_transcriptome_context.2026-06-14` | `external-unverifiable` | https://pubmed.ncbi.nlm.nih.gov/38049980/ | `present` | `metadata_only` | `2026-06-14` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.hmg_2019.zmiz1_dendritic_vitamin_d_context.2026-06-28` | `external-unverifiable` | https://academic.oup.com/hmg/article/28/2/269/5115479 | `present` | `metadata_only` | `2026-06-28` | missing-terms-url | `NOT_PROJECT_GROUNDED` |
| `claim.gwas_catalog.zmiz1_rs1250550_ms_crohn_opposite_alleles.2026-06-28` | `external-unverifiable` | https://www.ebi.ac.uk/gwas/rest/api/associations/search/findByRsId?rsId=rs1250550&projection=associationBySnp | `present` | `metadata_only` | `2026-06-28` | https://www.ebi.ac.uk/gwas/docs/about | `NOT_PROJECT_GROUNDED` |

## Interpretation

- `present` means the record has a complete `source_terms` object that passed the V48 linter.
- `missing_optional` is not a failure; it marks a future terms-review target.
- `metadata_only` is the conservative default for external resources unless a future record-specific review justifies more.
- This report does not authorize data redistribution and does not alter any grounded finding, locked rule, or pre-registration.
