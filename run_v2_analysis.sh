...
if [[ -s results_v2/acsl_family_inventory_summary.json ]]; then
  echo "Using cached results_v2/acsl_family_inventory_summary.json"
else
...
if [[ -s results_v2/nampt_feasibility_summary.json ]]; then
  echo "Using cached results_v2/nampt_feasibility_summary.json"
else
  ./.venv/bin/python scripts/v2_nampt_feasibility.py
fi
./.venv/bin/python scripts/v2_extended_autoimmune_checks.py
if [[ -s results_v2/prior_art_pubmed_counts.tsv ]]; then
  echo "Using cached results_v2/prior_art_pubmed_counts.tsv"
else
