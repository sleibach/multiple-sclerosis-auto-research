...
ld_labels <- ld_raw[[1]]
LD_full <- as.matrix(ld_raw[, -1])
rsids <- sub('_.*$', '', ld_labels)
rownames(LD_full) <- rsids
colnames(LD_full) <- rsids
summary_rows <- list()
for (gene in c('GPR25','DDX59','KIF21B','C1orf106')) {
  path <- file.path(out, paste0(gene, '_aligned_disease_eqtl.tsv'))
  ss <- read.delim(path, stringsAsFactors=FALSE)
  if (nrow(ss) < 50) {
    summary_rows[[length(summary_rows)+1]] <- data.frame(gene=gene, comparison='NA', status='too_few_snps', nsnps=nrow(ss), max_PP.H3=NA, max_PP.H4=NA, error='')
    next
  }
  labels <- ss$snp
  LD <- LD_full[labels, labels]
  ...
  for (comp in names(comps)) {
    ...
    set.seed(20260606)
    res <- tryCatch(coloc.susie(d_gwas, d_eqtl, susie.args=list(L=10, coverage=0.95, min_abs_corr=0.1, max_iter=1000)), error=function(e) e)
    if (inherits(res,'error')) {
      writeLines(conditionMessage(res), file.path(out, paste0(gene,'_',comp,'_eqtl_coloc_error.txt')))
      ...
      write.table(res$summary, file.path(out, paste0(gene,'_',comp,'_eqtl_coloc_summary.tsv')), sep='\t', quote=FALSE, row.names=FALSE)
      write.table(res$results, file.path(out, paste0(gene,'_',comp,'_eqtl_coloc_results.tsv')), sep='\t', quote=FALSE, row.names=FALSE)
...
write.table(rollup, file.path(out,'eqtl_susie_coloc_rollup.tsv'), sep='\t', quote=FALSE, row.names=FALSE)
print(rollup)
