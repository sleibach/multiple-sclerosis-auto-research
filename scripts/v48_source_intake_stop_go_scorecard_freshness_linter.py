#!/usr/bin/env python3
"""Check that the V48 source-intake stop/go scorecard is fresh."""
...
DEFAULT_TEMPLATE = ROOT / "knowledge_external/templates/SOURCE_INTAKE_STOP_GO_SCORECARD_V48.md"
DEFAULT_SUMMARY = ROOT / "knowledge_external/catalogs/indexes/source_intake_stop_go_scorecard_v48_summary.json"
DEFAULT_OUTDIR = ROOT / "analysis/v48_source_intake_stop_go_scorecard_freshness_linter"
...
REQUIRED_PHRASES = {
    ...
    "does_not_score_truth": "does not score scientific truth",
    "does_not_add_records": "add external records",
    "no_numeric_evidence_score": "Do not average criteria into a numeric evidence score",
    "no_model_override": "Do not let model/RPT output override a stop or park condition",
    "candidate_not_finding": "Do not treat `go_candidate_intake` as a finding",
    "routing_tool": "intake-routing tool",
    "not_project_change": "not whether a project conclusion should change",
}

EXPECTED_SUMMARY = {
    "markdown": "knowledge_external/templates/SOURCE_INTAKE_STOP_GO_SCORECARD_V48.md",
    "n_decision_outcomes": 5,
    "n_required_linked_controls": 8,
    "n_scorecard_criteria": 10,
    "overall_status": "PASS",
    "purpose": "V48 source-intake stop/go scorecard; template/navigation only; no biological claim",
}
...
def parse_args() -> argparse.Namespace:
    ...
    lint = sub.add_parser("lint", help="Lint source-intake stop/go scorecard freshness")
    ...
    synth = sub.add_parser("synthetic-check", help="Run synthetic stop/go scorecard fixtures")
...
def lint_template(template: Path, summary_path: Path, outdir: Path, fail_on_error: bool) -> int:
    ...
    for section in REQUIRED_SECTIONS:
        add(rows, f"section_present.{section}", "PASS" if section in sections else "FAIL", "required stop/go scorecard section")
    ...
    n_criteria = count_table_rows_after_section(text, "Scorecard Criteria")
    n_outcomes = count_table_rows_after_section(text, "Decision Outcomes")
    add(rows, "table_count.scorecard_criteria", "PASS" if n_criteria == 10 else "FAIL", f"observed={n_criteria} expected=10")
    add(rows, "table_count.decision_outcomes", "PASS" if n_outcomes == 5 else "FAIL", f"observed={n_outcomes} expected=5")
    ...
    write_tsv(outdir / "source_intake_stop_go_scorecard_freshness_lint.tsv", rows, ["check", "status", "detail"])
    result = {
        "synthetic": False,
        "purpose": "V48 source-intake stop/go scorecard freshness lint; template/navigation only; no biological claim",
        "n_checks": len(rows),
        "n_fail": n_fail,
        "overall_status": "PASS" if n_fail == 0 else "FAIL",
    }
    (outdir / "source_intake_stop_go_scorecard_freshness_lint_summary.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
...
def synthetic_check(outdir: Path, fail_on_error: bool) -> int:
    ...
    outdir.mkdir(parents=True, exist_ok=True)
    template = outdir / "synthetic_stop_go.md"
    ...
    summary.write_text(json.dumps({"markdown": "stale", "n_scorecard_criteria": 1, "n_decision_outcomes": 0, "overall_status": "FAIL"}) + "\n")
    lint_out = outdir / "synthetic_lint"
    lint_template(template, summary, lint_out, fail_on_error=False)
    rows = list(csv.DictReader((lint_out / "source_intake_stop_go_scorecard_freshness_lint.tsv").open(), delimiter="\t"))
    checks = {
        ...
        "bad_criteria_count_fails": any(row["check"] == "table_count.scorecard_criteria" and row["status"] == "FAIL" for row in rows),
        "bad_outcome_count_fails": any(row["check"] == "table_count.decision_outcomes" and row["status"] == "FAIL" for row in rows),
        "bad_summary_fails": any(row["check"] == "summary_matches.n_scorecard_criteria" and row["status"] == "FAIL" for row in rows),
    }
    check_rows = [{"check": check, "status": "PASS" if ok else "FAIL"} for check, ok in checks.items()]
    write_tsv(outdir / "synthetic_source_intake_stop_go_scorecard_freshness_checks.tsv", check_rows, ["check", "status"])
    result = {
        "synthetic": True,
        "purpose": "V48 source-intake stop/go scorecard freshness synthetic fixture; no biological claim",
        "n_checks": len(check_rows),
        "n_fail": sum(1 for row in check_rows if row["status"] != "PASS"),
        "overall_status": "PASS" if all(checks.values()) else "FAIL",
    }
    (outdir / "synthetic_source_intake_stop_go_scorecard_freshness_summary.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    print(json.dumps(result, indent=2, sort_keys=True))
