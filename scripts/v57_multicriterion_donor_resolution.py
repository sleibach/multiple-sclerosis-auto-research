#!/usr/bin/env python3
"""Resolve the 9-11 donor boundary for the frozen V57 rescue gate.

This script generates seeded synthetic method-test data only. It imports the
committed parent generator and changes only donor counts and replicate count.
"""
...
DEFAULT_OUTDIR = ROOT / "analysis/v57_multicriterion_donor_resolution"
DONOR_COUNTS = (9, 10, 11)
...
def main() -> int:
    ...
    resolved = min(passing) if passing else None
    summary = {
        "synthetic": True,
        "purpose": "donor-count resolution for a synthetic method gate; no biological claim",
        ...
        * parent.N_CANDIDATES,
        ...
        "first_tested_passing_donor_count": resolved if resolved is not None else 12,
        "resolved_within_9_11": resolved is not None,
        "n_gate_checks": len(gate_rows),
        "n_gate_fail": sum(row["status"] == "FAIL" for row in gate_rows),
        "overall_status": "PASS" if resolved is not None else "FAIL",
    }
    ...
    resolved_text = (
        f"`{resolved}` is the smallest tested donor count that passes every frozen check."
        if resolved is not None
        else "None of 9-11 donors passes every check; 12 remains the first tested passing count."
    )
    report = f"""# V57 Multi-Criterion Donor Resolution
...
There were `{summary['n_gate_fail']}` failed checks among
`{summary['n_gate_checks']}`. `gate_checks.tsv` preserves every cell-level
