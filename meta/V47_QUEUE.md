...
| 19 | 2026-06-13T20:08:41Z | 2026-06-13T20:14:29Z | done | Added 6 external-unverifiable landmark claim records for MS disease-course context and DMT mechanism-label context. External index PASS (`37` records), schema lint PASS (`692` checks), relationship lint PASS (`111` checks), uniqueness lint PASS (`74` checks after narrowing source-locator uniqueness to resource-catalog records), reachability PASS (`35` reachable/redirected-2xx, `2` maintenance warnings), provenance audit PASS (`334` checks, `37` external JSON records). |
| 20 | 2026-06-13T20:15:24Z | 2026-06-14T08:45:02Z | done | Added generated public `knowledge_external/INDEX.md`. Synthetic fixture PASS (`4/4` checks); real build PASS (`37` records, `7` navigation links). Governance refreshed: registry PASS (`130` scripts), markdown lint PASS (`10` Markdown files, `178` checks), provenance audit PASS (`334` checks, `37` external JSON records), stale detector PASS (`77` artifacts). The block target has been exceeded by the measured wall clock from the recorded block start. |
...
| 15 | External integration | MS-specific landmark external knowledge records for current disease-course and DMT mechanisms | done | Added 6 external-unverifiable records under `knowledge_external/records/`; all source-attached, `NOT_PROJECT_GROUNDED`, and orthogonal to project findings. |
...
| 28 | Navigation/index | Build public `knowledge_external/INDEX.md` from class/category/access-tier indexes | done | `scripts/v47_public_external_index.py`; generated index links only external-tree artifacts and repeats `NOT_PROJECT_GROUNDED` boundary. |
