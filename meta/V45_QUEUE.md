...
| 10 | Infrastructure/RPT | Exercise RPT on V44 structured readiness tables as proposal-only and verify no evidence claim changes | in-progress | RPT output must be grounded or labeled proposal-only. |
...
| 15 | Infrastructure | Implement real-cohort ingestion scripts for secondary postpartum and T/B schemas before opening matching data | todo | Generated by V45 item 9; current secondary harnesses are synthetic mechanics only. |
