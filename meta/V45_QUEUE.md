...
| 135 | Integrity | Add newest readiness checks into the generated checker registry | done | Regenerated `analysis/v45_generated_checker_registry/` after adding the received-package dry run, author-run schema validator, and OpenGWAS sentinel; registry now indexes `76` V45 scripts, with `0` undocumented and `0` without detected output directories. Added explicit output-dir exceptions for shortened generator names and fixed trailing-empty TSV fields. |
...
- Cross-route readiness linter completed. The live linter checks all `4`
  routes for request artifact, explicit blocker state, route arrival packet,
  no-scoring hard-stop text, and command-plan or author-run gate equivalent.
  Current status is `PASS` with `0` hard/soft issues. A synthetic
  missing-request fixture fails as expected with `1` hard issue. The stale
  detector now includes the linter and passes with `15` checked artifacts.
- Next selected task: external-request packet completeness and checksum guard.
- Outbound request packet integrity guard completed. It hashes the four live
  outbound request packets from the external blocker board, verifies unsent/
  approval guard language plus recipient/subject structure, and writes a
  manifest for handoff. Live status is `PASS` with `0` hard/soft issues;
...
- Next selected task: synthetic metadata-contradiction stress test for
  validation intake.
- Metadata-contradiction stress test completed. Synthetic clean metadata passes;
  within-patient response-label conflict, duplicate/misordered timepoint, and
  response-confounded batch fixtures all fail as expected. The audit is linked
  from validation intake and first-24h docs as an intake blocker before harness
  readiness, and its output directory is pre-classified as synthetic intake
  verification for the next governance wrapper.
- Next selected task: route-specific analyzable-pair calculator.
- Route-specific analyzable-pair calculator completed. Synthetic route-shaped
  cases classify small complete Gafson as effect-size/CI likely inconclusive,
  partial Gafson and small Karolinska as below planning floor, and GSE228330
...
- Cold-start operator command sequence generator completed. It joins the current
  action card, route-arrival packet index, received-package decision tree, and
  command-plan/author-return gate paths into one route-level operator sequence.
  Current generated state covers `4` routes, all with `may_score_now=no`; the
  sequence is linked as step 2 in the operational handoff. Stale-output detector
  now tracks `20` artifacts with `0` stale/missing; handoff path resolution
...
- Next selected task: generated checker registry integration for the newest
  readiness checks.
- Generated checker registry integration completed. The registry now indexes
  `76` V45 scripts, including the received-package dry run, author-run schema
  validator, and OpenGWAS sentinel, with `0` undocumented scripts and `0`
  scripts without detected output directories. Added explicit output exceptions
