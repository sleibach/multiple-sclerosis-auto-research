...
| 76 | Validation readiness | Add a harness-ready decision template that converts gate statuses into a yes/no run decision | todo | Generated after request/log templates; prevents running the harness with partial gates. |
| 77 | Integrity | Add command-plan consistency check into the pre-commit readiness wrapper | todo | Generated by item 73; makes command-plan drift part of the standard guard sequence. |
| 78 | Validation readiness | Add an author-run frozen harness packet template for cohorts that cannot share individual-level data | todo | Generated by author-label escalation matrix. |
| 79 | Validation reporting | Add minimum non-sensitive author-run output specification for external harness runs | todo | Generated by author-run option and validation report template. |
| 80 | Infrastructure | Add sample-ID canonicalization and de-identification rules template for received metadata repair | todo | Generated by failure taxonomy and subject-map risks. |
| 81 | Validation readiness | Add batch/QC/steroid metadata dictionary template for incoming cohorts | todo | Generated by V32/V44 confounder and batch guard requirements. |
| 82 | External account | Update collaborator package README to link all new V45 receipt, redaction, report, and handoff templates | todo | Generated by template expansion after item 60. |
| 83 | Infrastructure | Add latest governance refresh and precommit run after new templates are committed | todo | Generated by continuing V45 drift; keeps indexes current. |
